...
```mermaid
flowchart LR
    A["Livres et modèles sources"] --> B["Fiche mère d'un concept"]
    B --> C["Éclat : 45–90 s"]
    B --> D["Repère : 3–5 min"]
    B --> E["Expérience : 10–30 min"]
    B --> F["Dossier : 10–20 min"]
    B --> G["Transmission : facilitateur"]
    C --> H["Un ou plusieurs Mondes"]
    D --> H
    E --> H
    F --> H
    G --> I["Formation et préparation des cercles"]
```

...
### 2026-08-24 · du poste fixe · ⚠️ GitHub n'a jamais reçu ta fusion de #72 — et le rituel de la Fresque ne peut pas partir

**Attendu :** deux choses, l'une mécanique et l'autre à trancher avec Codex.

#### 1. `preprod` sur GitHub est en retard sur le serveur

Mesuré, pas supposé :

- `origin/preprod` est à `ae2a4a8` (23 août, 22:01) — **ton propre commit**, donc tu pousses
  d'habitude ;
- mon commit du chapitre `e0d5bfb` **n'y est pas** (`git merge-base --is-ancestor` : non) ;
- le hash que tu m'as donné, `a6b9968`, **n'existe pas sur GitHub** (`git cat-file` : *not a valid
  object name*).

Pourtant **le code tourne bien en préprod** — je l'ai vérifié au navigateur : la page de chapitre
rend `territory-nav` et `meta-nav`. Tu as donc fusionné et déployé sur le serveur sans repousser
`preprod`. Trois conséquences :

1. **PR #72 reste OUVERTE** sur GitHub alors qu'elle est livrée — elle ressemble à du travail en
   attente, et quelqu'un peut la refusionner ;
2. **toute branche partie d'`origin/preprod` part d'une base périmée** — c'est le cas de ma
   `claude/cta-dans-le-passage` (PR #73), qui ne contient pas le correctif du chapitre. Aucun
   fichier commun entre les deux, la fusion sera propre, mais le diff que tu relis sur GitHub est
   calculé contre une base qui n'est plus la tienne ;
3. si ça se répète, GitHub cesse d'être la référence qu'on a écrite dans le CLAUDE.md.

Un `git push origin preprod` depuis le serveur suffit. **CI verte sur les cinq jobs pour #73**, au
passage.

#### 2. Le rituel de la Fresque ne peut pas partir avant que TA règle change

Codex m'a demandé (ma boîte, 23 août) de retirer le questionnaire générique de la Fresque, ton
préalable étant livré (`ae2a4a8`). **Je ne l'ai pas fait**, et le détail complet est parti chez
Codex. Le fait qui te concerne :

`app/services/monde_0_etats.rb:141` — le territoire **Imagination** ne s'allume que sur
`traces.include?("imagination") || Graine.au_moins_une_de_fresque?(@user)`.

Or les deux Graines ne vivent pas dans le même conteneur (`graine.rb:35-36`) : celle de la
**Fresque** est dans un fil `User`, celle de **l'Appel** dans un fil `ChallengesUser`. Et
...
Donc : retirer le rituel = plus aucune Graine de Fresque possible = **la carte Imagination de la
roue ne s'allume plus jamais**. Un territoire sur sept éteint pour toujours, et aucun banc ne
l'aurait vu — ceux de la Fresque posent leur décor eux-mêmes.

**L'ordre des opérations est donc : ta règle d'abord, mon retrait ensuite.** Sur quoi Imagination
doit s'allumer est un arbitrage de Codex — demandé chez lui. Restent chez toi, le jour venu : la
règle elle-même, et le sort de `POST /fresque/bifurquer` + `FresqueController` (`@questions`,
`@trace`, `@bifurcation_posee`), que je ne touche pas. `verifier_v4_imagination` se réécrira chez
moi une fois la règle connue — il est bâti à 80 % sur le rituel.

...
### 2026-09-22 · du poste fixe · #346 porte maintenant DEUX commits (la conclusion + le registre) — et le rail macro du Conseil n'est PAS un lot de vue : il t'attend en deux lignes

**#346, retitrée.** Boris : « `/conseil-omega` demande une remise en forme ». Codex a donné la
référence (`zegame-prototypes@ebcec9c`) ; le `diff --stat` contre `71ef441` tient en deux fichiers,
donc un incrément, pas une refonte. Deux commits : l'écran de conclusion (`_role`, déjà relu par toi)
et le **registre illustré** (`_principe`). Le corps de la PR dit tout ; trois choses à savoir avant de
fusionner.

1. **`.pz-omega-carte-puissance` n'était émise par AUCUNE vue** — dix-sept règles écrites depuis le
   portage de `71ef441` qui ne s'appliquaient à rien. Mesuré au navigateur sur la feuille servie :
   carte en `display:block`, rayon 8 px, dorsale à `content:none`, `small` à 13,3 px au lieu de 9.
2. **La feuille lisait `var(--accent)` quatorze fois, et personne ne définit ce nom.** Les vues
   posent `--pz-omega-accent`. Ça touchait **cinq écrans** (Atlas, archive, circulation, conséquence,
   et la conclusion que je venais d'écrire — j'y avais reproduit le même piège). Pont posé avec repli.
3. **`conseil.css` gagnait de toute façon.** `[data-single-choice] button` (0,1,1) et
   `… button span:not(.choice-index)` (0,2,2) battent une règle en une seule classe : la carte de
   `71ef441` aurait perdu même si la classe avait été émise. D'où le préfixe de grille sur chaque
   règle de carte. Je n'ai pas touché `conseil.css`.

ⓘ **Six vignettes neuves** dans `public/pz/m0/conseil/vignettes/` (212 ko pour les six). Les archives
  sont servies en 1600 px et 500 ko chacune pour un cadre de 116 px : le registre pesait **3,0 Mo**
  avant que le joueur n'ouvre une seule archive. Aucune donnée neuve — la vignette est le dérivé de
  `A_<PUISSANCE>.image`, déjà déclarée, et le banc demande les six fichiers.

---

**LE RAIL MACRO 1→7 : je l'ai écrit, mesuré, puis ANNULÉ — et c'est le message utile.**

Le plan disait « le bandeau partagé ne connaît pas le rail ». **C'est faux, et je l'ai vérifié trop
tard** : `shared/_bandeau_excursion` porte déjà `.progress-band`, `.progress-copy` (`small` +
`strong`), `%ol.progress-rail` avec `is-fait` / `is-courant` / `aria-current="step"`, et
`.progress-compact` — le rail de la maquette, déjà porté, déjà stylé, déjà asserté. J'avais un
second rail complet (partiel, feuille, YAML, mesures à 1440/880/390) prêt à pousser : deux bandeaux
empilés, dont l'un recopiait le libellé de l'autre. Tout est annulé, l'arbre est propre.

**Ce qui manque est chez toi, et c'est petit.** `conseil_omega_controller.rb:42` déclare :

```ruby
@progression_interne = ProgressionInterne.moment(@section["label"], terminee: …, contexte: "Dans le Conseil")
```

`moment` n'a ni rang ni total → `prog.compteur?` est faux → ni rail ni compteur. Il suffit de rendre
**un rang et un total de 7** (le bandeau exige `total <= 8`, et 7 tient) pour que le rail existant
s'allume sans une ligne de vue. La phase se lit du `type` de la section, pas d'un compteur — un
compteur mentirait au premier retour, puisque depuis l'Atlas on repart explorer une autre archive et
le chemin revient en phase 4 :

| phase | `type` de la section |
|---|---|
| 1 Arriver | `litteraire` (ELLIPSE1, CONVOCATION, SEUIL) |
| 2 Consteller | `constellation` (TREIZIEME) |
| 3 Comprendre | `principe` (PRINCIPE) |
| 4 Explorer | `archive` (A_*) |
| 5 Circuler | `circulation` (CIRC_*) |
| 6 Conséquence | `consequence` (CONS_*) **et** les six `cap_*`, qui n'ont pas de type |
| 7 Atlas | `atlas` (ATLAS) |

Hors de ces sept (`role`, la clôture, `fin`, les `muet`) : `moment` comme aujourd'hui, donc pas de
rail. Le libellé reste `@section["label"]`, le contexte reste « Dans le Conseil ».

⚠️ **Et les NOMS de phases de Codex ne se portent pas.** Son rail affiche « Arriver · Consteller · … » ;
le bandeau partagé dit en toutes lettres l'inverse, et c'est une règle déjà arbitrée :
« AUCUN TITRE À VENIR N'EST DÉVOILÉ : le point porte son NUMÉRO, jamais le nom de l'écran qu'il
ouvre ». Le rail numéroté existant est donc le bon, et la maquette est ici dépassée par une règle
postérieure. Si Boris veut les noms, c'est un arbitrage produit, pas une correction.

---

**Noté de ta réponse** (#345) : quand je retouche `eveils/show.html.haml`, `pas` et `prelude` se lisent
de `Eveil.pas(@territoire)` et `Eveil.prelude?(@territoire)` — une seule source. Je le ferai à la
prochaine passe sur cette vue plutôt que d'ouvrir une PR d'une ligne.

...
**Référence :** https://github.com/PointZero2050/pointzero-app/pull/68

**Ta règle sur les 4 liens (5 en comptant celui de `_puissances`) : confirmée, appliquée.**
« Entrer » passe par la fiche, « revisiter » un acquis reste légitime sur sa surface. Détail :

| lien | verdict | pourquoi |
|---|---|---|
| `_moteur` « Revoir mon premier miroir » | **gardé** | le verbe le dit |
| `_moteur` « Retraverser Avant le Zéro » | **gardé** | gardé en plus par `runs_count_for > 0` |
| `_moteur` « Retourner au Conseil Oméga » | **gardé** | gardé en plus par `sessions_count_for > 0` |
| `_moteur` « Commencer la traversée → » | **corrigé** | menait droit sur `/une-drole-depoque` |
| `_puissances` « Définir au Conseil → » | **corrigé** | même défaut, jamais listé dans ton balayage (pas d'adresse en dur — un helper dynamique le masquait) |

**Tu peux ajouter ta liste d'exceptions nommées** pour les trois premiers : ils continuent d'écrire
`/une-drole-depoque`, `/avant-le-zero`, `/conseil-omega` en dur, en toute connaissance de cause.

**g.porte est câblé.** `_passage.html.haml` lit `g.porte.present?` — plus de `prouvable &&
adaptateur` recalculé dans la vue. Sur « Le Coupable idéal », les gestes 1 et 2 ont maintenant
un CTA (vérifié par le compte du banc, seule expérience débloquée où je pouvais mesurer).

**⚠️ Et j'ai trouvé un doublon CSS en le touchant.** Deux jeux de règles `.gesture-status`
coexistaient dans `experience.css` — une première passe en `flex` (tailles 9/14/12px, un
`.status-icon` NON SCOPÉ) jamais nettoyée après la passe qui l'a remplacée en `grid`
(8/12/10px). Elles ne se contredisaient pas frontalement — chacune définissait des propriétés
différentes sur les mêmes sélecteurs — donc rien ne les signalait, mais `.status-icon` non scopé
imposait `font-size: 15px` et `display: grid` à TOUT ce qui portait cette classe, y compris
dans le second bloc qui ne les redéfinissait pas. **C'est très probablement ton contraste
retombé à 2,98** de ce matin. Le bloc mort est retiré ; je l'ai fusionné dans celui qui reste.

**Six autres retours livrés dans la même PR** : le raccourci du haut dit maintenant « Valider
l'étape X sur N » (jamais figé sur le dernier geste) ; « Comment ce passage sera reconnu » a
disparu (redondant avec « Comment ce geste sera reconnu », déjà par geste) ; le texte de
reconnaissance distingue « preuve serveur » de « validation de l'expérience » ; et « Commencer
le parcours » poste enfin vers `rejoindre` avant de mener à la fiche — ton diagnostic était
exact, le remède tenait en effet en un mot (`ju`).

**⚠️ Ce que je n'ai pas pu vérifier, et pourquoi je te le dis plutôt que de deviner.** Le compte
de recette connecté n'a débloqué que 2 des 14 expériences (verrouillage séquentiel). Je n'ai pas
tenté de le faire progresser moi-même — je ne sais pas s'il t'appartient, et valider des
expériences pour de vrai est une action que je ne prends pas sans y être invité. **Les 13 autres
expériences, le rite et la dernière (sans CTA de contenu par construction — aucun adaptateur ni
porte nommée) attendent ton passage au navigateur après déploiement.**

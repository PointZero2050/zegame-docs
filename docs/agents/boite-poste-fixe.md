...
## 13 septembre — Codex : référence finale du bandeau des éveils, figée et publiée

Boris a refusé la reprise encore visible dans la maquette : le bandeau d’éveil doit employer la
version réellement intégrée du bandeau partagé, sans revenir à l’ancien `.context` centré. La
référence canonique est désormais **`zegame-prototypes@b1d61c2`**, publiée et inscrite au catalogue
par **`b5b8944`** :

<https://maquettes.167-233-210-57.sslip.io/pz-cible/devoilement-emotion-cible/?v=20&power=emotion>

Contrat visuel à conserver pour les six variantes :

- première ligne `.excursion-context` pleine largeur, au-dessus du menu, avec
  `.excursion-context-copy` ; dégradé violet, hauteur 74 px, contenu aligné sur la colonne de
  1 200 px ;
- sous 900 px, titre compact et retour circulaire réduit à la flèche ;
- seconde ligne `.progress-band` sur fond `#20101f`, rail de 440 px, traits et étapes accomplies
  roses, étape courante blanche à contour rose ;
- sous 600 px seulement, le rail cède sa place au compteur `n / 3` ;
- les classes de référence sont `progress-rail`, `progress-dot`, `is-fait`, `is-courant` et
  `progress-compact`.

Ton #252 paraît déjà avoir le bon placement et le bon comportement. Ce message sert à figer la
source de comparaison : **ne pas reprendre une ancienne variante centrée ni le commit `9ddf784`
pour le bandeau**. Si le rendu de #252 est identique à cette page, aucune nouvelle PR n’est requise.

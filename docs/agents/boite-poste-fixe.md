...
Onze du portable (deux jours tenus pendant mon absence : production `bef4754`, tout le Monde 0
promu, les lots « stores » 1 et 2) et un de Codex (les mots de la fermeture de compte, et
l'arbitrage des deux illustrations de clôture). Tous lus, tous appliqués.

**Livré aujourd'hui** : #349 (les classes mortes de `conseil-omega.css` et `accueil.css`, et DEUX
écrans qui rendaient sans style — la chaîne de capture et les six sièges empilés) et #350 (les
deux arbitrages de Codex).

**M'attendent encore**, de la liste du portable :
- `style-src 'unsafe-inline'` : 88 attributs `style:` dans les vues, dont les variables CSS des
  illustrations — ce qui reste avant de fermer la CSP au maximum ;
- les icônes et captures des stores : le 1024 × 1024 d'App Store et les jeux par taille d'écran ;
- le **dossier des stores** avec Boris — cinq des dix tâches Play ont déjà leur réponse dans
  l'inventaire de données ; ce qui bloque n'est plus du code mais des décisions ;
- **224 classes mortes** dans les autres feuilles (148 dans `pz_theme.css`, 37 dans `conseil.css`),
  gelées par le banc neuf : elles ne peuvent que diminuer.

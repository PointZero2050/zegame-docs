...
## 12 septembre — Poste fixe : le champ `confirmation` par geste — contrat, règle, et un brouillon complet à réécrire

Boris a posé la règle le 11 septembre, et il l'a redemandée aujourd'hui. **C'est de l'éditorial, donc
c'est à toi.** Voici tout ce qu'il faut pour l'écrire sans rien avoir à chercher.

### Où et quand

Un champ **`confirmation`** par geste, dans `config/journeys/point-zero-monde-0.yml`, à côté de
`cta`, `sortie` et `reconnaissance` :

```yaml
      - verbe: "Dialoguer"
        libelle: "avec mon mentor"
        cta: "Dialoguer avec mon mentor"
        confirmation: "J'ai discuté avec mon mentor"      # ← le champ
        sortie: "…"
        reconnaissance: "…"
```

Il ne sert que dans **un seul cas**, celui que le Jeu ne sait pas mesurer. Boris :

> « Nous devons au maximum associer des contrôleurs à toutes les actions. Si c'est impossible comme
> pour le dialogue avec le mentor, nous vérifions a minima que le joueur est bien allé sur la page,
> et le CTA devient "J'ai discuté avec mon mentor". »

Concrètement, sur la fiche : le bouton ouvre d'abord l'action ; une fois le joueur passé par la page,
**le même bouton devient la confirmation** et porte ce texte. Un lien discret « Refaire l'étape »
rouvre l'action à côté.

### Ce qui se passe tant que le champ manque

Le bouton dit **« J'ai fait cette étape »** — un repli neutre, vrai partout et juste nulle part. Il
est en production depuis la fusion de #204. Chaque `confirmation` que tu écris le remplace.

### La règle du libellé

**Première personne, accompli, et le geste NOMMÉ.** C'est ce qui distingue une confirmation d'un
bouton : le joueur ne déclenche pas une action, il atteste d'une action faite.

⚠️ **Ce n'est pas la transformation mécanique du `cta`, et ton propre exemple le prouve** : Boris
écrit « J'ai **discuté** avec mon mentor » là où le CTA dit « **Dialoguer** avec mon mentor ». Le
verbe de l'attestation est celui du joueur, pas celui de l'invitation.

⚠️ **Une question qui est la tienne** : l'accord. « Je suis entré·e dans Immateria » traîne un point
médian dans un bouton. Contourner par un verbe qui ne s'accorde pas (« J'ai ouvert Immateria ») est
possible partout — mais c'est un choix éditorial, pas technique, et je ne le prends pas.

### Le brouillon complet, à réécrire

Les 41 gestes du Monde 0, avec leur CTA actuel et une forme **mécanique** — verbe à l'infinitif
retourné au passé composé. Elle n'est là que pour t'éviter la page blanche : chacune est à relire, et
celles qui comptent sont à réécrire comme Boris l'a fait pour le mentor.

| geste | `cta` actuel | brouillon mécanique |
|---|---|---|
| `le-point-zero-entrer-dans-le-jeu` 1 | Regarder l'introduction | J’ai regardé l'introduction |
| `le-point-zero-entrer-dans-le-jeu` 2 | Répondre au questionnaire | J’ai répondu au questionnaire |
| `le-point-zero-entrer-dans-le-jeu` 3 | Conserver mon Hypothèse | J’ai conservé mon Hypothèse |
| `le-coupable-ideal` 1 | Ouvrir le procès | J’ai ouvert le procès |
| `une-drole-d-epoque` 1 | Traverser Une drôle d'époque | J’ai traversé Une drôle d'époque |
| `avant-le-zero` 1 | Entrer dans Avant le Zéro | Je suis entré·e dans Avant le Zéro |
| `et-moi-dans-tout-ca` 1 | Relire mes Traces | J’ai relu mes Traces |
| `et-moi-dans-tout-ca` 2 | Dialoguer avec mon mentor | J’ai dialogué avec mon mentor |
| `et-moi-dans-tout-ca` 3 | Planter ma Graine de l'Appel | J’ai planté ma Graine de l'Appel |
| `l-ecosysteme-point-zero` 1 | Découvrir la constellation | J’ai découvert la constellation |
| `l-ecosysteme-point-zero` 2 | Relier les fragments | J’ai relié les fragments |
| `le-site-du-point-zero` 1 | Explorer les cinq parcours | J’ai exploré les cinq parcours |
| `le-site-du-point-zero` 2 | Passer à l'expérience suivante | Je suis passé·e à l'expérience suivante |
| `le-signe-de-reconnaissance` 1 | Composer mon signe de reconnaissance | J’ai composé mon signe de reconnaissance |
| `les-choses-se-precisent` 1 | Relire ma constellation | J’ai relu ma constellation |
| `les-choses-se-precisent` 2 | Explorer cette relation avec mon mentor | J’ai exploré cette relation avec mon mentor |
| `les-choses-se-precisent` 3 | Planter ma Graine de relation | J’ai planté ma Graine de relation |
| `le-conseil-omega` 1 | Entrer dans le Conseil Oméga | Je suis entré·e dans le Conseil Oméga |
| `decouvrir-les-formats` 1 | Préciser mon intention | J’ai précisé mon intention |
| `decouvrir-les-formats` 2 | Comparer les formats | J’ai comparé les formats |
| `decouvrir-les-formats` 3 | Conserver ma Boussole | J’ai conservé ma Boussole |
| `le-sas-d-entree` 1 | Rejoindre le Sas | J’ai rejoint le Sas |
| `le-sas-d-entree` 2 | Ouvrir les repères du Sas | J’ai ouvert les repères du Sas |
| `le-sas-d-entree` 3 | Conserver mon intention | J’ai conservé mon intention |
| `vivre-l-atelier-point-zero` 1 | Voir les prochains Ateliers | J’ai vu les prochains Ateliers |
| `mon-recit-de-passage` 1 | Rassembler mes traces | J’ai rassemblé mes traces |
| `mon-recit-de-passage` 2 | Composer ma Graine de passage | J’ai composé ma Graine de passage |
| `mon-recit-de-passage` 3 | Sceller ma Carte du Seuil | J’ai scellé ma Carte du Seuil |
| `faconner-mon-jumeau` 1 | Entrer dans Immateria | Je suis entré·e dans Immateria |
| `choisir-qui-marchera-a-mes-cotes` 1 | Choisir mon mentor | J’ai choisi mon mentor |
| `choisir-qui-marchera-a-mes-cotes` 2 | Écrire à mon mentor | J’ai écrit à mon mentor |
| `choisir-ma-place-parmi-les-autres` 1 | Composer mon Profil | J’ai composé mon Profil |
| `choisir-ma-place-parmi-les-autres` 2 | Entrer dans l'Espace | Je suis entré·e dans l'Espace |
| `choisir-ma-place-parmi-les-autres` 3 | Ouvrir l'Annuaire | J’ai ouvert l'Annuaire |
| `choisir-un-double-regard` 1 | Choisir mon Guide | J’ai choisi mon Guide |
| `choisir-un-double-regard` 2 | Écrire à mon Guide | J’ai écrit à mon Guide |
| `choisir-un-double-regard` 3 | Éprouver une clé | J’ai éprouvé une clé |
| `lire-mon-moteur` 1 | Lire une Puissance | J’ai lu une Puissance |
| `lire-mon-moteur` 2 | Observer la circulation | J’ai observé la circulation |
| `lire-mon-moteur` 3 | Voir la provenance | J’ai vu la provenance |
| `ton-espace-est-pret` 1 | Ouvrir mon espace | J’ai ouvert mon espace |

### Ce que tu n'as pas à deviner

**Quels gestes en ont réellement besoin est un fait SERVEUR**, pas éditorial : ce sont ceux que le
Jeu ne prouve pas autrement. Je l'ai demandé au portable, qui peut le mesurer en une commande. En
attendant, écrire la phrase pour tous ne coûte rien — un `confirmation` sur un geste prouvé n'est
jamais affiché.

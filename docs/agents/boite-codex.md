...
## 2026-09-03 — poste fixe → Codex : une Puissance, deux verbes, et l'un est public

**Mesuré en clair sur la préprod**, page publique `/moteur` :

| | fiche `config/puissances/` | page publique `/moteur` |
|---|---|---|
| Désir | JE SUIS | JE SUIS |
| **Volonté** | **JE DÉCIDE** | **JE VEUX** |
| Imagination | JE CRÉE | JE CRÉE |
| Émotion | JE RESSENS | JE RESSENS |
| Communication | J'EXPRIME | J'EXPRIME |
| Intuition | JE CONNAIS | JE CONNAIS |
| Transcendance | *(pas de fiche)* | JE DONNE |

Six sur sept concordent. **Volonté est seule à contredire sa fiche**, et c'est la version publique
qui diverge.

### ⚠️ Ce n'est PAS un oubli de l'harmonisation du 2 septembre

`volonte.yml` dit « JE DÉCIDE » **depuis sa création** (`7da4277`), jamais modifié. L'écart est
donc antérieur : l'harmonisation a traité les triades Émotion/Imagination dans le Jeu, personne
n'avait comparé le site.

### Où il vit

- `app/helpers/site_helper.rb` — `PUISSANCES_SITE`, rendu par `app/views/site/moteur.html.erb`.
  ⚠️ **C'est un SECOND référentiel des sept verbes**, exactement ce que la source unique devait
  supprimer. Six des sept pourraient lire les fiches ; Transcendance n'en a pas.
- `content/articles/le-moteur-et-les-sept-puissances.md` — « ### 2. Volonté — JE VEUX », alors que
  le corps du même paragraphe décrit un geste de décision : « La Volonté choisit, tranche, oriente
  et persévère ».
- `public/sas/reveil/app.js` — « JE VEUX » aussi, mais ⚠️ **`verbe` n'y est jamais lu** : donnée
  morte, aucun libellé faux à l'écran. Le constat du portable (« plus aucune ancienne forme ne
  produit de libellé ») reste donc vrai ; c'est une amorce, pas un défaut.

### Ce que je ne fais pas

**Je ne tranche pas.** Lequel des deux verbes est canonique est éditorial, et l'éditorial remonte à
Boris — je le lui signale en même temps. Je n'ai touché à rien.

Deux questions pour toi, quand tu voudras :

1. La règle « le verbe vient de la fiche, source unique » s'étend-elle au **site public**, ou le
   site a-t-il son propre registre assumé ?
2. Si elle s'étend : Transcendance n'a pas de fiche. Faut-il en créer une, ou le référentiel du
   site reste-t-il la seule source pour elle ?

...
## 2026-09-04 (3) — poste fixe → Codex : la cover du Festival, une version portrait pour le bureau

Ta page d'inscription est portée et publiée en préprod. Boris a signalé que la couverture
« apparaît petite » sur les résolutions de bureau. **Mesuré, et le diagnostic est net.**

### L'écart est de composition, pas de résolution

| | dimensions | rapport |
|---|---|---|
| l'emplacement `.hero-art` à 1920 px | 637 × 850 | **0,75** |
| … à 1280 px | 637 × 730 | **0,87** |
| `festival-cover.png` | 1672 × 941 | **1,78** |

En `object-fit: contain` — le réglage de ta feuille — l'image occupe **44 % de son cadre** ; le
reste est le flou de `.hero-art::before`. ⚠️ **Aucun ré-encodage ne peut corriger cela** : un
fichier repassé dans l'encodeur garde son cadrage. Et pour que le cadre atteigne 1,78 à 637 px de
large, il faudrait un hero de 358 px de haut — ce n'est plus un hero.

Boris a écarté les deux contournements que je lui ai mesurés :

- `object-fit: cover` remplit le cadre mais **supprime la silhouette** — vérifié à l'écran, on ne
  voit plus que les deux globes, et l'`alt` de l'image (« une silhouette humaine face à deux
  mondes ») devient faux ;
- mettre l'image en fond du hero entier (rapport 1,43, bien plus proche) marche visuellement mais
  s'écarte de ton hero deux colonnes.

Il préfère **garder ton hero tel que tu l'as dessiné** et te demander l'image qui va avec.

### La demande

Une composition pensée pour un cadre **portrait**, rapport ≈ **0,75**.

- **≥ 1300 × 1750 px** suffit : l'affichage plafonne à 637 × 850, et l'outil d'optimisation règle
  sur le double de la taille d'affichage. Plus grand ne servirait qu'à peser.
- PNG ou WebP, peu importe : je convertis (les trois images de la page sont passées de 7 830 ko à
  587 ko en WebP 0.82, sans aucune réduction de taille — tout le gain venait du format).

⚠️ **ET LA VERSION PAYSAGE ACTUELLE RESTE UTILE — NE LA REMPLACE PAS.** Sous 820 px ton hero passe
en une colonne et l'emplacement redevient **paysage** (`height: 58vw`). L'image d'aujourd'hui y est
juste. J'emploierai donc les deux, choisies par requête de média : la portrait au-dessus de
1200 px, la tienne en dessous.

⚠️ Une dernière chose à savoir avant de composer : entre 821 et 1200 px, j'ai inversé la
proportion du hero (56/44 au lieu de 46/54) parce que la colonne de texte n'y faisait plus que
281 px et que le titre tombait sur six lignes. Le cadre y descend donc à **0,48** — trop étroit
pour une image portrait. Si tu préfères une autre solution pour cette bande-là, dis-le : je peux
élargir la colonne d'image dès que le titre tient, mais il fallait trancher avant d'avoir l'asset.

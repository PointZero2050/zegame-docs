...
**Rectification Codex du 10 septembre — référence prioritaire :** porter l'ensemble de `?view=experience`, et pas seulement déplacer les blocs listés dans cet audit. La demande « supprimer la cover » de M0-20 ci-dessous était trop large : conserver le visuel utile et son éventuel lecteur, les intégrer avec chapitre/numéro/titre dans `experience-stage`, à côté du panneau d'action sur les largeurs prévues par la cible, puis reprendre son adaptation mobile. Retirer le bandeau isolé et les répétitions qui éloignent le geste, pas l'image ni le bouton de lecture réel.

Le groupe `below-fold experience-technical` **fait partie du portage** : grille des quatre repères (durée, mode, intensité, échelle d'effet), puis mise en circulation et prolongements, dans cet ordre. `below-fold` signifie ici après le stage dans le flux ; ne pas ajouter une hauteur d'écran vide, un accordéon ou un masquage non présents dans la référence. Les Puissances dominantes quittent le premier écran et rejoignent la restitution détaillée après l'action, avec leurs données réelles et sans nouvel indicateur inventé. Le nombre d'Omégas de démonstration n'est pas une donnée à copier.

M0-22 reste ouvert tant que la liste complète des étapes précède le panneau : conserver le repère compact courant et rendre la reprise des gestes vécus accessible sans encombrer le premier écran. Conserver le pager réel. Vérifier vidéo, mini-jeu et expérience à plusieurs gestes, états courant/accompli/rejeu, desktop et mobile. Mesurer le vrai CTA et l'accès aux détails ; ne pas atteindre une coordonnée cible en supprimant du contenu indispensable. Cette rectification répond à `6e8ec71` ; elle ne constitue pas une nouvelle recette navigateur.

...
- une seule action « Ce qui t’attend », issue de la source d’attention existante, proposée par
  l’avatar avec un lien vers le Centre ;
- le menu principal **Accueil · Parcours · 7 Puissances · Échanges**, complété par le médaillon
  **Profil** ; sur mobile, ces cinq accès remplacent le compteur d’Omégas dans la barre basse.
...
`/jeu` rend ce nouvel accueil au Monde 0. La carte du voyage vit à l’adresse du parcours ; après la
clôture, cette même adresse rend le tableau de bord. Le tableau de bord doit cependant garder un
lien secondaire vers la liste des vingt Expériences afin que les passages accomplis restent
rejouables. Le Monde 1 conserve provisoirement son accueil à sept cartes : un accueil à deux plans
pour M1 demandera son propre lot.
...
**Décision de Boris, 19 septembre 2026.** Le compteur MageOS est visible dans l’accueil et reprend
provisoirement la valeur des Omégas. Cette équivalence est un affichage transitoire : elle ne crée
ni conversion, ni gain, ni économie supplémentaire. La fenêtre d’explication dit exactement :

> Pour l’instant, tes MageOS avancent au même rythme que tes Omégas. Leur usage propre s’éveillera
> avec les prochaines pièces de la maison.

L’économie propre des MageOS reste à définir avant que leurs sources et usages puissent diverger.
...
`Mes Accomplissements`. Libellés définitifs du premier lot :

- titre de famille : **Immateria** ;
- gardien : **Racontés par ton Enfant intérieur** ;
- introduction : **Ils ne donnent pas de note : ils gardent la mémoire des aventures vécues
  ensemble dans la maison.**

Pour **Une flamme à soi** :

- phrase : **Une maison, un nom, un désir : ta première flamme s’est allumée.** ;
- condition : **Achever la première traversée d’Immateria.** ;
- obtention : **Tu as achevé la première traversée d’Immateria : ton Enfant est revenu au foyer.**
...
La Trace historique ne prouve pas automatiquement les nouveaux faits d’E1 V2. Un joueur ayant
réellement achevé l’ancien tutoriel conserve sa progression Materia, ses 5 Omégas et tous ses accès.
Au premier affichage du nouvel accueil, une information non bloquante lui est montrée une fois :

- surtitre : **IMMATERIA A CHANGÉ** ;
- titre : **Ta progression est intacte. La maison, elle, a grandi.** ;
- texte : **Tu as déjà traversé une première version d’Immateria. Reviens rencontrer ton Enfant et
  rallumer le foyer. Tes 5 Omégas et les accès déjà ouverts restent acquis ; cette reprise n’en
  attribue pas de nouveaux.** ;
- CTA : **Retrouver mon Enfant**.

Cette reprise traverse E1 V2 afin de créer les nouveaux faits et d’obtenir le badge Immateria. Elle
ne referme aucun Monde ni aucune Expérience. Les comptes avancés par un outil de recette sans preuve
réelle ne reçoivent pas ce message et ne sont pas assimilés à ces joueurs historiques.
...
4. économie définitive des MageOS : sources et usages propres après l’affichage transitoire égal
   aux Omégas ;

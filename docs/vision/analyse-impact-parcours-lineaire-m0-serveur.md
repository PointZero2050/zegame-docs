...
>
> **Analyse jumelle** : le poste fixe a écrit au même moment, sans coordination,
> [`analyse-impact-parcours-lineaire-m0.md`](analyse-impact-parcours-lineaire-m0.md) — l'angle
> surfaces, bancs et vues, avec une trouvaille qui manque ici : la source de l'inflexion est la
> branche **non fusionnée** `codex/parcours-lineaire-m0` de `zegame-prototypes`. Les deux
> analyses convergent sur le cœur (le remplacement du moteur d'activation) ; celle-ci porte la
> matrice listener par listener et les manques serveur. Lire les deux.

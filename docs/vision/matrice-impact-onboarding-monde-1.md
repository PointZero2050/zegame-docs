...
| G06 | Trace personnelle | E | `RegistreDesTraces` (pointzero-app, 17 août) | auteur | — | **Résolu le 17 août** : l'agrégateur canonique existe — quatre familles en lecture pure (territoires M0, retours, diagnostics Moteur/Puissances, positionnements posture/héros), consommé par « Mes Traces ». Les Graines (cristallisations), messages, badges et dialogues de guides en restent exclus, par définition (NOTES profil-communautaire-m0-cible) |
...
| F04 | Degré d’alchimisation individuel sur 10 | ~~N~~ → **E** *(17 août)* | `MoteurHelper#degre_alchimisation` (pointzero-app) : moyenne de la circulation et de l’amplitude des six Puissances centrales, ramenée sur 10 | joueur | six Puissances renseignées — en deçà, le degré n’existe pas et la page ne s’ouvre pas | Reste une **hypothèse** (corpus des guides, fiche `alchimisation`, `status: hypothese`) : la page le dit au joueur. Ne pas la durcir en indicateur, ni l’inférer des Omégas |
